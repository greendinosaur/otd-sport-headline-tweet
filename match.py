...
class Match:
    ...
    def __init__(self, date, competition, competition_round, opponent, place):
        """ initialises the main fields of Match
            ...
            competition - the name of the competition the match was part of
    ...
    def set_result_data(self, result, score, normal_time="NT", match_report_url=""):
        """ sets details about the actual result between the team and the opponent
            ...
            normal_time - a flag indicates whether the score was at the end of normal time ("NT"),
                            extra time ("AET") or penalty shoot-out ("PST")
...
def save_matches_to_file(fname_out, matches):
    """ saves a list of match(es)
        ...
        otherwise will append the data

        fname_out - file to save the matches to
        matches - a list of match to save

    """
    with open(fname_out, 'a', newline='', encoding='utf8') as file:
        writer = csv.writer(file)
        for output_match in matches:
            writer.writerow([output_match.date.strftime("%Y-%m-%d"),
                             output_match.competition,
                             output_match.competition_round,
                             output_match.place,
                             output_match.opponent,
                             output_match.result,
                             str(output_match.score[0]) + "-" + str(output_match.score[1]),
...
def load_matches_data(date_of_interest, fname):
    """ load the matches data and returns those whose date (day and month)
        ...
        date_of_interest - the month and day of the matches to load
        fname - the file name whose data will be loaded
    """
    ...
    for line in file_in:
        ...
        if match_date.day == date_of_interest.day and match_date.month == date_of_interest.month:
            ...
            loaded_match = Match(match_date,
                                 ...
                                 match_details[4],
                                 match_details[3])
            loaded_match.set_result_data(match_details[5],
                                         tuple([int(n) for n in match_details[6].split('-')]),
                                         match_details[7],
                                         match_details[8].rstrip())
